...
let randomize () =
  ...
  let c = W.connect ~priority:W.Join rnd_button result on_press Trigger.buttons_down in

  (* the layout *)
  let minmax = L.flat_of_w ~align:Draw.Center ~background:(L.color_bg (Draw.(transp grey)))
                 [label_min; ask_min; label_max; ask_max] in
  ...
  let board = Main.make [c] [layout] in
  Random.self_init ();
  Main.run board;;
